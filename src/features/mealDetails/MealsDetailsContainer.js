...
const mapStateToProps = state => {
  const selectedMeal = state.foods.selectedMeal || {
    details: [],
    mealName: ''
  };
  ...
  copyOfFoodsOfMeal.map(object => {
    for (let key in selectedMeal.details) {
      if (selectedMeal.details[key].id === object.id) {
        object.carbs = (object.carbs *
          selectedMeal.details[key].quantity /
          100).toFixed(2);
        object.protein = (object.protein *
          selectedMeal.details[key].quantity /
          100).toFixed(2);
        object.fat = (object.fat *
          selectedMeal.details[key].quantity /
          100).toFixed(2);
      }
    }
  ...
  let mealDetails = copyOfFoodsOfMeal.reduce(
    (total, food) => {
